...
<div class="page-header">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h2>Edit Reminder</h2>
            <p>Perbarui pengingat tugas kamu</p>
        </div>
...
<div class="dashboard-card" style="max-width:660px">
    ...
    <form action="{{ route('reminder.update', $reminder) }}" method="POST" class="form-styled">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="tugas_id" class="form-select @error('tugas_id') is-invalid @enderror">
                ...
                @foreach($tugas as $t)
                    <option value="{{ $t->id }}" {{ old('tugas_id', $reminder->tugas_id) == $t->id ? 'selected' : '' }}>
                        {{ $t->judul }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="reminder_date"
                   ...
                   value="{{ old('reminder_date', \Carbon\Carbon::parse($reminder->reminder_date)->format('Y-m-d\TH:i')) }}">
        ...
        <div class="mb-4">
            ...
            <select name="status" class="form-select @error('status') is-invalid @enderror">
                <option value="Aktif"    {{ old('status', $reminder->status) == 'Aktif'    ? 'selected' : '' }}>Aktif</option>
                <option value="Nonaktif" {{ old('status', $reminder->status) == 'Nonaktif' ? 'selected' : '' }}>Nonaktif</option>
            </select>
            @error('status')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>

        <button type="submit" class="btn btn-primary w-100" style="padding:11px">
            <i class="bi bi-save me-2"></i>Update Reminder
        </button>
    </form>
</div>
