...
<div class="dashboard-card" style="max-width:700px">
    ...
    <form action="{{ route('catatan.update', $catatan) }}" method="POST" class="form-styled">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="isi" rows="10"
                      class="form-control @error('isi') is-invalid @enderror">{{ old('isi', $catatan->isi) }}</textarea>
            @error('isi')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>

        <button type="submit" class="btn btn-primary w-100" style="padding:11px">
            <i class="bi bi-save me-2"></i>Update Catatan
        </button>
    </form>
</div>
